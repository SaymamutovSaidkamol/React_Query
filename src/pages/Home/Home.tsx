...
import useGetUser, { type responseT, type UserT } from './service/query/useGetUser';
...
const Home = () => {
  ...
  const [dataRegion, setDataRegions] = useState<responseT | null>(null)
  ...
  useEffect(() => {
    setDataRegions(data?.regions?.data);
  }, [data])
  ...
  const handleSearch = () => {
    if (inputRef.current) {
      ...
      const filtered = data?.regions?.data.filter((item) => item.name.toLowerCase().includes(searchText))
    ...
    <>
      <div className='container mx-auto mt-20 flex justify-center'>
        <div className=' flex items-center bg-[#eee] rounded-[5px]'>
          <IoIosSearch onClick={handleSearch} className='text-[42px] px-2 cursor-pointer' />
          <input className='outline-none' type="text" placeholder='search...' ref={inputRef} />
        ...
        isLoading ? <div>Loading...</div> : (
          <>
            ...
            <div className='container mx-auto  mt-10 flex justify-center'>
              <Stack spacing={2}>
                <Pagination count={data?.regions.totalPages} onChange={handleChangePage} variant="outlined" />
              </Stack>
            </div>
          </>
        )
